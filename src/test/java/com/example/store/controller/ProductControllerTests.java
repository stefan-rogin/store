...
class ProductControllerTests {
    ...
    void findByresId() throws Exception {
        mockMvc.perform(get("/products/a18920fb-56cd-41c5-8264-ed617c038524"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Two"))
                .andExpect(jsonPath("$.resId").value("a18920fb-56cd-41c5-8264-ed617c038524"))
    ...
    @Test
    @Transactional
    void upsertExistingProduct() throws Exception {
        String THREE_RESID = "410d3bbb-67f1-479c-81b0-a852e6579eb4";
        mockMvc.perform(put("/products/" + THREE_RESID)
                .header("Content-type", "application/json")
                .content("{\"name\": \"ThreeChanged\", \"price\": {\"amount\": 3.39, \"currency\": \"RON\"} }"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("ThreeChanged"))
                .andExpect(jsonPath("$.resId").value(THREE_RESID))
                .andExpect(jsonPath("$.price.amount").value(3.39))
                .andExpect(jsonPath("$.price.currency").value("RON"));
        mockMvc.perform(get("/products/" + THREE_RESID))
                .andExpect(jsonPath("$.name").value("ThreeChanged"))
                .andExpect(jsonPath("$.resId").value(THREE_RESID))
                .andExpect(jsonPath("$.price.amount").value(3.39))
                .andExpect(jsonPath("$.price.currency").value("RON"));
    ...
    @Test
    @Transactional
    void upsertNewProduct() throws Exception {
        String FOUR_RESID_NEW = "6887bf30-c00c-4897-8423-c155bbff076b";
        mockMvc.perform(put("/products/" + FOUR_RESID_NEW)
                .header("Content-type", "application/json")
                .content("{\"name\": \"Four\", \"price\": {\"amount\": 4.39, \"currency\": \"EUR\"} }"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Four"))
                .andExpect(jsonPath("$.resId").value(FOUR_RESID_NEW))
                .andExpect(jsonPath("$.price.amount").value(4.39))
                .andExpect(jsonPath("$.price.currency").value("EUR"));
        mockMvc.perform(get("/products/" + FOUR_RESID_NEW))
                .andExpect(jsonPath("$.name").value("Four"))
                .andExpect(jsonPath("$.resId").value(FOUR_RESID_NEW))
                .andExpect(jsonPath("$.price.amount").value(4.39))
                .andExpect(jsonPath("$.price.currency").value("EUR"));
